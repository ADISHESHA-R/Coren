...
- **Public customer feedback**: **`/customer-feedback/{siteId}`** — anonymous form (no JWT). The POST body defaults to `{ "feedbackJson": "<stringified fields>" }` to match the admin API shape; set **`VITE_PUBLIC_CUSTOMER_FEEDBACK_BODY_MODE=flat`** if your backend expects a flat JSON body. Configure the URL with **`VITE_PUBLIC_CUSTOMER_FEEDBACK_POST_URL_TEMPLATE`** if it differs from the default (see `.env.example`).
