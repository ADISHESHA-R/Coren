...
- **Customer feedback (Render env):** For the tokenless public form, do **not** set **`VITE_PUBLIC_CUSTOMER_FEEDBACK_TOKEN_REQUIRED=true`** at build time unless you intentionally require invite links. Set **`VITE_DEV_STUB_PUBLIC_CUSTOMER_FEEDBACK=false`** (or remove it) on production so the real API is used. After backend changes to **GET …/customer-feedback**, redeploy the static site so the latest bundle parses **`data`** correctly.
